    ...
    if request.method == "POST":
        if get_text("musical") == "All Musicals":
            if get_text("musicalSongs") and not get_text("nonMusicalSongs"):
                ...
                songs.genre, songs.musicalNon FROM songs INNER JOIN musicals ON musicals.Id = songs.musicalId WHERE \
                ...
                songList = db.execute("SELECT musicals.musical, songs.song_title, songs.role, songs.singerArtist, songs.original_musical, songs.composer, \
                songs.genre, songs.musicalNon FROM songs INNER JOIN musicals ON musicals.Id = songs.musicalId WHERE songs.pendingNon != 'pending'")
...
def review():
    if request.method == "POST":
        if session["user_id"] == 1:
            ...
            songList = db.execute("SELECT musicals.musical, songs.song_title, songs.role, songs.singerArtist, songs.genre, songs.original_musical, songs.composer, \
